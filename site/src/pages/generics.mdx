...
You're finally ready to learn a little secret:

`functions:values::generics:types` (functions are to values as generics are to
types)

To put it another way: generics are the functions of the types world.
...
Returning to our `Box` example, it means we can use generic types to enable:
...
Now how do we get `getThingFromBox` to be aware of that?
...
Keep learning and playing and practicing! Don't be afraid to ask for help!

And most importantly: try and **teach what you're learning to others**! Teaching
is a great way to discover where your knowledge is limited! And someone out
there is a little bit behind you in skill and knowledge and would love to read
about what you're learning right now.

You'll find yourself feeling much more confident with them in no time.
...
#### Creating types that map from type `A` to type `B`
